...
import {
  getAuthTokenForRegistry,
  getRegistryForPackage,
  isNpmRegistry,
} from "../clients/util/getNpmUrl";
...
export async function getPackageMetadata(packageName: string, wsRoot?: string) {
  try {
    ...
    if (!cachedMetadata) {
      ...
      outputChannel.appendLine(`Got metadata for package ${packageName}`);
    ...
    outputChannel.appendLine(
      `Error getting metadata for package ${packageName}: ${getErrorMessage(err)}`,
    );
  }
}
