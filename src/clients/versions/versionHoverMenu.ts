                ...
                for (const [line, decoration] of decorationRanges) {
                    if (decoration.range.contains(position)) {
                        ...
                        markdown.appendMarkdown(
                            `[Latest Version Update](${createUpdateButton(decoration.packageName, decoration.latest.version, position.line, line)}): ${decoration.latest.version} published ${createReleaseDate(decoration.latest.release)}\n\n`
                        );
                        markdown.appendMarkdown(
                            `[Safe Version Update](${createUpdateButton(decoration.packageName, decoration.safest.version, position.line, line)}): ${decoration.safest.version} published ${createReleaseDate(decoration.safest.release)}`
                        );
